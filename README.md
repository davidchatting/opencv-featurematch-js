...
// -> { valid: true, transform: [16 numbers, column-major 4x4], transform2D: [a, b, c, d, e, f], inlierMatches: [...26 pairs...], outlierMatches: [...], reason: 'OK' }
```
...
`transform` and `transform2D` are the same homography in two shapes: `transform` is padded to a flat 16-element **column-major** 4x4 (the same layout WebGL/OpenGL use natively, so `applyMatrix(...result.transform)` in p5.js's WEBGL mode works directly, no conversion needed - as does `drawProjectedImage()` and the other 4x4 matrix helpers below), `transform2D` is the flat 6-element `[a, b, c, d, e, f]` affine form that both the canvas API's `setTransform()` and p5.js's `applyMatrix()` (2D mode) expect directly - equivalent to calling `to2dAffine(transform)` yourself, just already done for you (perspective terms are dropped in `transform2D`). Invert it with `invertMatrix2D`, not `invertMatrix4x4`. Both are populated as soon as `Align_img` finds a homography at all - including on an otherwise-`invalid` result (e.g. one `isReasonableHomography` rejected for excessive perspective) - so you can inspect or use a rejected transform yourself rather than only ever getting `null`. They're only `null` when no homography was found in the first place.
